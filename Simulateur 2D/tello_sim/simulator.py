...
class Simulator():
    ...
    def check_takeoff(self):
        if not self.takeoff_state:
            ...
            print(self.name,"is flying at {} centimeters above my takeoff altitude.".format(self.altitude))
            pass
    ...
    def plot_altitude_steps(self, e):
        ...
        ax.plot(self.altitude_data,'ro', linestyle='dashed', linewidth=2, markersize=6)
        ...
        # C = [-150, -50, -150, -150]
        # D = [-150, -150, -50, -150]
        # plt.plot(C, D, label='Triangle')

        # P = [7, 100, 100, 7, 7]
        # Q = [25, 25, 150, 150, 25]
        # plt.plot(P, Q, label='Square')
        
        # # Plot object
        # U = [-100]
        # V = [-75]
        # ax.plot(U, V, 'md', markersize=6, label='Object')
    ...
    def plot_horz_steps(self, e):
        ...
        # xlowlim = -50 if xlow > -50 else xlow - 40
        # xhilim = 50 if xhi < 50 else xhi + 40
        # ylowlim = -50 if ylow > -50 else ylow - 40
        # yhilim = 50 if yhi < 50 else yhi + 40
        ax.set_xlim([xlowlim, xhilim])
        ax.set_ylim([ylowlim, yhilim])
        ax.plot(horz_df[0], horz_df[1], 'bo', linestyle='dashed', linewidth=2, markersize=6, label="Drone Moves")
        ax.plot(horz_df[0], horz_df[1], linewidth=e, alpha=.15)
        if len(self.flip_coors) > 0:
            flip_df = pd.DataFrame(self.flip_coors)
            ax.plot(flip_df[0], flip_df[1], 'ro', markersize=6, label="Drone Flips")
        ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        ax.grid()
        ax.legend()
    ...
    #     C = [-150, -50, -150, -150]
    #     D = [-150, -150, -50, -150]
    #     Triangle = plt.plot(C, D, label = 'Triangle')

    #     P = [7, 100, 100, 7, 7]
    #     Q = [25, 25, 150, 150, 25]
    #     Square = plt.plot(P, Q, label = 'Square')
        
    #     Obstacle = Triangle or Square
   ...
   # Movement Commands
    def takeoff(self):
        ...
        if not self.takeoff_state:
            ...
            print("The estimated takeoff altitude for", self.name,"is {} centimeters".format(self.altitude))
        else:
    ...
    def land(self, e=25):
        ...
        print("Here are the graphs of your flight for", self.name,"! I can't wait to try this for real.")
    ...
    def up(self, dist: int, e=25):
        ...
        print("The current bearing of", self.name,"is {} degrees.".format(self.bearing))
        self.altitude = self.altitude + dist
        self.altitude_data.append(self.altitude)
        new_loc = self.dist_bearing(orig=self.cur_loc, bearing=self.bearing, dist=self.altitude)
    ...
    def left(self, dist: int, e=25):
        ...
        new_loc = tuple(s)
        self.path_coors.append(new_loc)
        # v = list(self.path_coors)
        # v[0] = int(v[0])
        # v[1] = int(v[1])
        # self.path_coors = tuple(v)
        # c = (i[0] for i in self.path_coors)
        # c = (i[1] for i in self.path_coors)
        # self.path_coors = tuple(c)
        print(self.path_coors)
    ...
    def cw(self, degr: int):
        ...
        print("New bearing for", self.name,"is {} degrees.".format(self.bearing))
