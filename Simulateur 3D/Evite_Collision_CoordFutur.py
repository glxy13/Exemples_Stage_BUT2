# -*- coding: utf-8 -*-
"""
Created on Wed Feb 22 07:42:59 2023
...
import os
count = 0 
newpath = "Resultats_Evite_Collision"
if not os.path.exists(newpath):
    os.makedirs(newpath)
newpath1 = "Resultats_Evite_Collision" + "/CoordFutur_"+str(count)
if not os.path.exists(newpath1):
    os.makedirs(newpath1)
newpath2 = "Resultats_Evite_Collision" + "/CoordFutur_"+str(count) + "/Images"
...
def forward_ext2ext(my_drone, cur, ext, pas1, t):
    while cur < ext:
...
def forward_ext2ext2(my_drone, cur, ext, pas1, t):
    while cur > ext:
        print("Strategie1 is running")
        collision(my_drone, my_drone1, pas1, 45) and collision(my_drone, my_drone2, pas1, 45) and collision(my_drone, my_drone3, pas1, 45)
        if (collision(my_drone, my_drone1, pas1, 45) or collision(my_drone, my_drone2, pas1, 45) or collision(my_drone, my_drone3, pas1, 45)) == True:
...
def Strategie1(my_drone, x_max, x_min, y_max, y_min):
    ...
    if etat_init == etat_curr: 
        ...
        while ((my_drone.cur_loc[0] >= ((x_max+x_min)/2)+round(pas1/2)) or (my_drone.cur_loc[0] <= ((x_max+x_min)/2)-round(pas1/4))) and ((my_drone.cur_loc[1] >= ((y_max+y_min)/2)+round(pas1/4)) or (my_drone.cur_loc[1] <= ((y_max+y_min)/2)-round(pas1/2))):
            ext = ext2[1]
            cur = etat_curr[1]
            forward_ext2ext(my_drone, cur, ext, pas1, 1)
            my_drone.cw(90)
            ext = ext3[0]
            cur = my_drone.cur_loc[0]  
            forward_ext2ext(my_drone, cur, ext, pas1, 2)
            my_drone.cw(90)
            ext = ext4[1]
            cur = my_drone.cur_loc[1]  
            forward_ext2ext2(my_drone, cur, ext, pas1, 3)
            my_drone.cw(90)
            ...
            ext = ext1[0]
            cur = my_drone.cur_loc[0]  
            forward_ext2ext2(my_drone, cur, ext, pas1, 4)
            my_drone.cw(90)
...
def forward_x2x(my_drone1, cur, x_min, x_max, pas2, t):
...
def forward_y2y(my_drone1, cur1, y_max, pas2):
...
def Strategie2(my_drone1, x_min, x_max, y_min, y_max):
    ...
    if etat_init == etat_curr: 
        ...
        while my_drone1.cur_loc != ext:
            cur = my_drone1.cur_loc[0]
            forward_x2x(my_drone1, cur, x_min, x_max, pas2, 1)
            cur1 = my_drone1.cur_loc[1]
            forward_y2y(my_drone1, cur1, y_max, pas2)
            my_drone1.cw(180)
            cur = my_drone1.cur_loc[0]
            forward_x2x(my_drone1, cur, x_min, x_max, pas2, 2)
            my_drone1.cw(180)
            cur1 = my_drone1.cur_loc[1]
            forward_y2y(my_drone1, cur1, y_max, pas2)
...
def forward_lim2lim(my_drone2, pas3, angle):
...
def Strategie3(my_drone2, x_min, x_max, y_min, y_max):
    ...
    if etat_init == etat_curr: 
        while my_drone2.cur_loc[0] < x_max:
            forward_lim2lim(my_drone2, pas3, angle)
        print("Drone3 a fini de parcourir la salle.")
    else:
        print("Drone n'est pas a la position initiale!")
    
# Avancer de bas jusqu'au haut et de droite à gauche pour le 4e drone
def forward_lim2lim2(my_drone3, pas4, angle):
    angle_degrees = abs(180 - angle)
    angle_radians = math.radians(angle_degrees)
    angle_compa = math.tan(angle_radians)
    
    if (my_drone3.cur_loc[0] - round(pas4*angle_compa)) <= x_min:
        ...
        while my_drone3.cur_loc[1] > y_min:
            if (my_drone3.cur_loc[1] - pas4) < y_min:
...
def Strategie4(my_drone3, x_min, x_max, y_min, y_max):
        ...
        if etat_init == etat_curr: 
            ...
            while my_drone3.cur_loc[0] > x_min:
                forward_lim2lim2(my_drone3, pas4, angle)
...
plot_all_in_a_graph(my_drone, my_drone1, my_drone2, my_drone3, 'rP', 'gP', 'cP', 'yP', 15)